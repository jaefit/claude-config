...
- **Claude Code 안에서 읽는 경우** — 아래를 직접 실행한다.
- **Claude 데스크톱/웹 앱에서 읽는 경우** (명령 실행 불가) — 사용자에게 **한 단계씩** 명령을
  불러주고, 각 단계의 출력을 받아 다음 단계를 판단한다. 한꺼번에 다 던지지 말 것.
  0단계를 마치면 그 다음부터는 Claude Code 안에서 이 문서를 다시 읽게 하는 편이 빠르다.
